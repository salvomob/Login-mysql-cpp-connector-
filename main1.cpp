...
int main(){
	char *usnm = new char[80];
	char *pasw = new char[80];
	char *server_db = new char[80];
	char *username_db = new char[80];
	char *password_db= new char[80];
	char *database = new char[80];
	
	std::cout << "Inserisci il tuo username database: ";
	std::cin >> username_db;	
	std::cout << "Inserisci il tuo server :";
	std::cin >> server_db;		
	std::cout << "Inserisci la tua password:";
	std::cin >> password_db;
	std::cout << "Inserisci nome database:";
	std::cin >> database;
	Connection con(server_db,username_db,password_db,database);
	std::cout << "Inserisci username applicativo: ";
	std::cin >> usnm;
	std::cout << "Inserisci la tua password:";
	std::cin>>pasw;
    MYSQL *connection;
    ...
    result = execute_query(connection,"select * from logins;");//logins is the table in myAccounts database, here goes yours!!
    int counter = 0;
    bool checkUs = false;
    bool checkPw = false;
